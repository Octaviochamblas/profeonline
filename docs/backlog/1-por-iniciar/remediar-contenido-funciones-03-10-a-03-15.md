...
2. **`al_terminar_debes_poder`** (333 recursos, 100%): 1-2 frases con la meta de aprendizaje.
3. **`afirmaciones_verdaderas`** (333 recursos, 100%): agregar hasta llegar a mínimo 2
   afirmaciones ciertas, breves y verificables (hoy todos tienen solo 1).
4. **`errores_frecuentes`** (330 de 333): completar hasta exactamente 5 afirmaciones falsas.
5. Cargar en DB local (`load_node_content`) y crear una migración de datos nueva, siguiendo el
...
- [ ] Script de verificación (adaptar `scratch/audit_antigravity_12secciones.py`) da 0 fallas
      para los 333 recursos de `03.10`–`03.15` en los 4 campos de la tabla de la auditoría.
