...
def publish_studio(request):
    if request.method == "POST":
        ...
        levels = Level.objects.filter(id__in=[lid for lid in level_ids if lid])
        ...
        if not skip_playlist and not playlist_id:
            errors["playlist_id"] = "Debe ingresar el ID de playlist o marcar 'Subir sin playlist'."
        ...
        if errors:
            ...
            context["selected_levels"] = [int(lid) for lid in level_ids if lid.isdigit()]
