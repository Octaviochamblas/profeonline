...
class SeedContentCommandTests(TestCase):
    def test_seed_content_command_is_idempotent(self):
        ...
        self.assertEqual(Topic.objects.count(), 10)
        self.assertEqual(Resource.objects.filter(is_published=True).count(), 6)
        self.assertEqual(Module.objects.filter(is_published=True).count(), 3)
